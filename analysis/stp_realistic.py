# Aggregate the original realistic OD into realistic STP. Need to first run 1) Dijkstra Solver 2) Revisit Solver
        ...
        for aDate, item in aggregationObject.items(): # Aggregating
            ...
            for stopID, stopItem in item.items(): # For all aggregated stops
                for budget in budgetList: # Divide by the day count to generate the average PPA
                    if stopItem["count"] == 0:
                        continue
                    stopItem["PPA_RV_" + str(budget)] /= stopItem["count"]
                    stopItem["PPA_SC_" + str(budget)] /= stopItem["count"]
                    stopItem["PPA_RT_" + str(budget)] /= stopItem["count"]
                insertList.append(stopItem)
            
            if insertList != []:
                client.cota_access_agg["REA_" + aggDate + "_" + str(i)].drop()
                client.cota_access_agg["REA_" + aggDate+ "_" + str(i)].insert_many(insertList)
